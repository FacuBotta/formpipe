...
**⚠️ Security Note:** For production deployments, it's recommended to use environment variables instead of storing SMTP credentials in `formpipe.config.json` or the generated PHP file.

The generated `contact-form.php` automatically reads the following environment variables if they are set (otherwise falls back to config values):

- `FORMPIPE_SMTP_HOST` - SMTP server hostname
- `FORMPIPE_SMTP_PORT` - SMTP server port
- `FORMPIPE_SMTP_USER` - SMTP username
- `FORMPIPE_SMTP_PASS` - SMTP password
...
1. Set environment variables on your production server (see `php/ENV.md` for detailed instructions)
...
**Note:** The generated `php` folder includes a `.gitignore` that prevents committing `contact-form.php` with credentials. See `php/ENV.md` for complete documentation on setting up environment variables for different server configurations (Apache, Nginx, Docker, etc.).
